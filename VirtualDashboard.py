...
file1 = "C:\\Users\\User\\Desktop\\Practice Coding\\PythonProjects\\FedEx Office Project\\NewGitProject\\combined_merged_geocoded_addresses.csv"

# Load the original dataset
df = pd.read_csv(file1, dtype=str)

# Remove the pattern from the 'Name' column
df['Name'] = df['Name'].astype(str)
# Create a new column 'Name_Address' that concatenates 'Name' and 'InputAddress'
df['Name_Address'] = df['Name'] + df['InputAddress']
...
# Convert 'Distance' column to float, ignoring errors for non-numeric values
...
def update_results(selected_distance_input):
    try:
        ...
        result_table = filtered_df[['Name', 'Distance', 'Website', 'InputAddress',
                                    'Phone', 'Rating', 'Reviews','Category', 'TripAdvisor Rating', 'TripAdvisor Reviews']].copy()  # Create a copy of the DataFrame
        ...
        return [html.H3(f"Total number of businesses within the selected distance: {total_businesses}"),
                ...
                html.Div(style={"height": "50px"}),  # Add some space
                html.Iframe(srcDoc=m2, style={"width": "100%", "height": "600px"})]
    except ValueError:
        return html.Div("Please enter a valid numeric distance.")
    except Exception as e:
        print(f"An error occurred: {e}")
        return html.Div("An error occurred. Please check the console for details.")
